...
- В задаче есть блоки жёстких фактов (одпис KRS, владельцы CRBR,
  финансы) - используй их как ГОТОВЫЙ контекст: масштаб и динамику
  компании бери из финансового блока, реорганизации (слияния/
  поглощения/разделения) из dział 6 - это важнейшее событие для
  раздела «Что изменилось», владельцы из CRBR подсказывают, кто
  реально принимает решения. Не ищи эти данные заново.
...
«нанимают трёх разработчиков интеграций (pracuj.pl, дата) → вероятно,
строят интеграционный слой и им не хватает рук/экспертизы». Только
гипотезы с основанием.
...
def _task_prompt(
    ...
    return (
        ...
        "главной). Официальный PKD уже дан в фактах выше - сверь с ним "
        "описание.\n\n"
