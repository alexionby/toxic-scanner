...
«нанимают трёх разработчиков интеграций (pracuj.pl, вакансии активны)
...
def _jobs_block(jobs: JobsResult) -> str:
    ...
    Источник вторичный (поисковая выдача формата pracuj.pl). Дат
    публикации в выдаче нет: «активна» - оценка по индексу (карточка
    есть в индексе pracuj.pl или проиндексирована за месяц), не
    подтверждённая дата - агент не должен выдавать её за точную.
    """
    if not jobs.ok:
        # Причина - в последней заметке адаптера (нет ключа / сбой запросов
        # / краш): пробрасываем, чтобы outage и мисконфиг различались.
        reason = jobs.notes[-1] if jobs.notes else "сбой поиска"
        return (
    ...
    # Частичная деградация: часть запросов упала - охват неполный.
    partial = ""
    if jobs.failed_queries:
        partial = (
            f" ВНИМАНИЕ: {jobs.failed_queries} из 3 поисковых запросов не "
            "удались, охват неполный - отсутствие вакансий не доказано."
        )
    if not jobs.vacancies and not jobs.unconfirmed:
        # Честность охвата: парсер понимает формат заголовков pracuj.pl;
        # вакансии на justjoin.it/nofluffjobs и т.п. осядут в «нераспознано».
        gap = ""
        if jobs.unparsed_count:
            gap = (
                f" ({jobs.unparsed_count} заголовков выдачи не распознано - "
                "возможны вакансии на порталах не в формате pracuj.pl)"
            )
        return (
            "Активные вакансии собраны автоматически (вторичный источник - "
            f"поисковая выдача, формат pracuj.pl): не найдено ни одной{gap}."
            f"{partial} Пиши «активных вакансий не найдено», для малых "
            "компаний это нормально; сам вакансии НЕ ищи и не выдумывай.\n\n"
        )
    lines = [
        "Активные вакансии (вторичный источник - поисковая выдача; "
        "«активна» = карточка есть в индексе pracuj.pl или "
        "проиндексирована за последний месяц, точных дат публикации нет). "
        f"Используй их как есть, сам вакансии НЕ ищи.{partial}"
    ]
    if jobs.vacancies:
        lines.append("Подтверждённые (город совпал с адресом компании):")
        for v in jobs.vacancies[:_JOBS_DISPLAY_CAP]:
            liveness = "в индексе, вероятно активна" if v.fresh else "давность неизвестна"
            lines.append(
                f"- {v.title} - {v.city or 'город не указан'} ({liveness}; {v.url})"
            )
    ...
    if jobs.unconfirmed:
        lines.append(
            "Под вопросом (город не совпал с адресом компании или не "
            "определён - возможен филиал или тёзка; упоминай только с "
            "оговоркой, не как факт):"
        )
        for v in jobs.unconfirmed[:_JOBS_DISPLAY_CAP]:
